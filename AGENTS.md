...
- This checkout contains FUXI-owned Producer changes. The public
  `infiniflow/ragflow` remote is reference-only and must not be synchronized
  by default. Do not `fetch`, `pull`, merge, or rebase it unless the user
  explicitly requests that operation in the current task.
- FUXI source synchronization, commits, and pushes must target only the
  `qqbinbinbin/ragflow` remote on `main`. Never commit or push FUXI changes to
  the public upstream remote.
- If upstream comparison is explicitly requested, perform it read-only and
  keep FUXI-specific files and local changes intact; do not automatically
  merge upstream history.
