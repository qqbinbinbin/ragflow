...
def structure_generation_ref(document_id: str, binary: bytes) -> str:
    ...
    from rag.app.tabular_structure import PRODUCER_SCHEMA_VERSION
    ...
    return str(uuid.uuid5(uuid.NAMESPACE_URL, f"fuxi:tabular-generation:{PRODUCER_SCHEMA_VERSION}:{document_id}:{source_sha256}"))
    ...
    *,
    ...
    try:
        ...
        try:
            ...
            # A concurrent final task may have activated the same deterministic
            # generation. Re-read before treating the CAS race as a failure.
            if _active_generation_ref(
                ...
                raise
        return {"status": "active", "producer_generation_ref": generation_ref, "row_count": len(projection["rows"])}
    except Exception as error:
        try:
            if _active_generation_ref(
                service,
